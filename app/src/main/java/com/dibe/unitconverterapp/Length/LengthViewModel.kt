...
class LengthViewModel : ViewModel() {
    ...
    private fun updateTextTwo() {
        val medium = state.textOne.value.toDoubleOrNull()
            ?.times(LengthVariables.conversionFactorsToKM[state.unitOne.value] !!)
        if (medium != null) {
            state.textTwo.value =
                (medium * LengthVariables.conversionFactorsFromKM[state.unitTwo.value] !!).toString()
            if (!isDecimal) {
