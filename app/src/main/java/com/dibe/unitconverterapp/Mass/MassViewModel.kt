...
class MassViewModel : ViewModel() {
    ...
    private fun updateTextTwo() {
        val medium = state.textOne.value.toDoubleOrNull()
            ?.times(MassVariables.conversionFactorsToKM[state.unitOne.value] !!)
        if (medium != null) {
            state.textTwo.value =
                (medium * MassVariables.conversionFactorsFromKM[state.unitTwo.value] !!).toString()
            if (!isDecimal) {
