...
class VolumeViewModel : ViewModel() {
    ...
    private fun updateTextTwo() {
        val medium = state.textOne.value.toDoubleOrNull()
            ?.times(VolumeVariables.conversionFactorsToStandard[state.unitOne.value] !!)
        if (medium != null) {
            state.textTwo.value =
                (medium * VolumeVariables.conversionFactorsFromStandard[state.unitTwo.value] !!).toString()
            if (!isDecimal) {
