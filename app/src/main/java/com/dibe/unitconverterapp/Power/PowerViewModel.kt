...
import com.dibe.unitconverterapp.Power.PowerState
...
class PowerViewModel : ViewModel() {
    ...
    private fun updateTextTwo() {
        val medium = state.textOne.value.toDoubleOrNull()
            ?.times(PowerVariables.conversionFactorsToStandard[state.unitOne.value] !!)
        if (medium != null) {
            state.textTwo.value =
                (medium * PowerVariables.conversionFactorsFromStandard[state.unitTwo.value] !!).toString()
            if (!isDecimal) {
