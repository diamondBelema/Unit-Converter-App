...
class EnergyViewModel : ViewModel() {
    ...
    private fun updateTextTwo() {
        val medium = state.textOne.value.toDoubleOrNull()
            ?.times(EnergyVariables.conversionFactorsToKM[state.unitOne.value] !!)
        if (medium != null) {
            state.textTwo.value =
                (medium * EnergyVariables.conversionFactorsFromKM[state.unitTwo.value] !!).toString()
            if (!isDecimal) {
